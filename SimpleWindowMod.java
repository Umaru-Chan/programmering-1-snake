 ...
 *
 *          SimpleWindowMod ---!!!OBSESRVERA!!!--- Denna SimpleWindows skiljer
 *          sig ifrï¿½n SimpleWindows orginal, se rad 213, 244, 249 och 269.
 *
 */
public class SimpleWindowMod {
	...
	private SWCanvasMod canvas; // the drawing area
	 ...
	 */
	public SimpleWindowMod(int width, int height, String title) {
		...
		canvas = new SWCanvasMod(width, height);
	...
	public void waitForMouseClick() {
		do {
			waitForEvent();
		} while (getEventType() != MOUSE_EVENT); // ï¿½ndrat
	}
	...
	public void waitForEvent() {
		...
		try {
			lastEvent = eventQueue.poll(100, TimeUnit.MILLISECONDS); // Vï¿½ntar
																		// 100
																		// ms
																		// pï¿½
																		// hï¿½ndelse
...
/* The drawing area */
class SWCanvasMod extends JPanel {
	...
	public SWCanvasMod(int width, int height) {
		super();
		this.width = width;
		this.height = height;
